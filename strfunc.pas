...
{ Обрезать/расширить строку по ширине }
function ClipStr(AString: AnsiString; AWidth: Integer): AnsiString;

{ Выравнивание строки по левому краю }
function LeftAlignStr(AString: AnsiString): AnsiString;
{ Выравнивание строки по правому краю }
function RightAlignStr(AString: AnsiString): AnsiString;
...
var
  len: Integer;
begin
  len := LazUTF8.UTF8Length(AString);
  if len >= AWidth then
    Result := LazUTF8.UTF8Copy(AString, 1, AWidth)
  else
    Result := AString + StringOfChar(' ', AWidth - len);
end;

{ Выравнивание строки по левому краю }
function LeftAlignStr(AString: AnsiString): AnsiString;
var
  len, strip_len: Integer; 
begin
  len := Length(AString);
  Result := StripStr(AString);
  strip_len := Length(Result);

  if strip_len < len then
    Result := Result + StringOfChar(' ', len - strip_len);
end;
...
begin
  ...
  if strip_len < len then
    Result := StringOfChar(' ', len - strip_len) + Result;
end;
